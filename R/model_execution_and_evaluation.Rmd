...
n_cell_lines <- max(subset_perfect_data$cell_line)

fit_real <- sampling(model_orig,
                     list(cell_lines = n_cell_lines,
                             proteins = n_prot,
                             prot_intensity = array(c(subset_perfect_data|>
                                                        arrange(ProteinGroup, rep, n_cell_lines) |>
                                                        pull(prot_intensity)), dim = c(n_reps,n_cell_lines,n_prot)),
                             replicates = n_reps),
                     iter = 1000,
                     chains = 4)
...
ggplot( tibble(gen_prot_intensity=c(apply(extract(fit_real,"gen_prot_intensity")$gen_prot_intensity,c(2,3,4),median))) |>
         mutate(rep = rep(1:n_reps, n_cell_lines*n_prot),
                ...
                protein_id = rep(1:n_prot, each = n_cell_lines*n_reps)) |>
         inner_join(subset_perfect_data)) +
...
real_post_posterior_check <- fit_summary_real |> 
  ...
  filter(str_detect(parameter, "m_C")) |>
  transmute(chrom = median)) |>
  mutate(cell_line = rep(1:n_cell_lines, each = n_prot),
         protein = rep(1:n_prot, n_cell_lines))
