...
fit_real <- sampling(model_orig,
                     list(cell_lines = subsampled_real_data_list$cell_lines,
                             proteins = subsampled_real_data_list$P,
                             input_enrichment = subsampled_real_data_list$input_enrichment,
                             prot_intensity = array(c(subsampled_real_data_list$prot_intensity), dim = c(2,55,50)),
                             replicates = n_reps),
                     iter = 1000,
                     chains = 4)
...
ggplot(fit_summary_real |> 
         filter(str_detect(parameter, "enrichment")) |>
         mutate(rep = rep(1:2, each = 55),
                cell_line = rep(1:55, 2)) |>
...
ggplot( tibble(gen_prot_intensity=c(apply(extract(fit_real,"gen_prot_intensity")$gen_prot_intensity,c(2,3,4),median))) |>
         mutate(rep = rep(1:2, 50*55),
                cell_line = rep(rep(1:55, each = 2), 50),
                protein_id = rep(1:50, each = 110)) |>
         inner_join(sam_test_dataset_simple)) +
  geom_point(aes(x = gen_prot_intensity, y = prot_int))  +
  labs(x="gen_prot_intensity")
...
real_post_posterior_check <- fit_summary_real |> 
  ...
  filter(str_detect(parameter, "m_C")) |>
  transmute(chrom = median)) |>
  mutate(cell_line = rep(1:55, each = 50),
         protein = rep(1:50, 55))
