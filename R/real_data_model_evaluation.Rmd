...
ggplot(named_protein_results) + 
 ...
 base_protein_results <- protein_dictionary |> 
  inner_join(fit_real |> 
  spread_draws(cyto_intensity_protein_base[protein_id],
               chrom_intensity_protein_base[protein_id]) |>
  summarise_draws())
 
ggplot(base_protein_results) + 
  geom_point(aes(x=ProteinGroup, y=log(median), colour = variable))
...
compare_localisation <- protein_dictionary |> 
  ...
  spread_draws(chrom_intensity_sum[cell_line,protein_id],
               cyto_intensity_sum[cell_line,protein_id]) |>
  summarise_draws()) |> 
  ...
  group_by(Uniprot,
            cell_line) |>
  summarise(variable = variable[1:2],
            final.assignment = final.assignment[1:2],
            median = median[1:2]) |>
  pivot_wider(names_from = variable,
              values_from = median)

ggplot(compare_localisation |> filter(cell_line == 5)) +
  geom_boxplot(aes(x = final.assignment, y=cyto_intensity_sum)) +
  theme_bw() +
...
ggplot(compare_localisation |> filter(cell_line == 5)) +
  geom_boxplot(aes(x = final.assignment, y=chrom_intensity_sum-cyto_intensity_sum)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.x = element_blank(),
        panel.grid.minor.y = element_blank())
...
ggplot(compare_localisation |> filter(cell_line == 5)) +
  geom_boxplot(aes(x = final.assignment, y=chrom_intensity_sum)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.x = element_blank(),
        panel.grid.minor.y = element_blank())
