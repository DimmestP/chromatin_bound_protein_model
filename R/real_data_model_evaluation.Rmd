...
named_protein_results <- protein_dictionary |> 
  inner_join(fit_real |> 
  spread_draws(total_intensity_protein_base[tissue,protein_id]) |>
  summarise_draws()) 
...
compare_localisation <- protein_dictionary |> 
  inner_join(fit_real |> 
  spread_draws(cyto_chrom_ratio[cell_line,protein_id]) |>
  summarise_draws()|>
  transmute(cell_line,
            protein_id,
            cyto_chrom_ratio = median)) |>
         inner_join(cell_line_dictionary) |> 
...
ggplot(compare_localisation |>
  ...
  geom_boxplot(aes(x = final.assignment, y=cyto_chrom_ratio)) +
  theme_bw() +
