...
named_protein_results <- protein_dictionary |> 
  inner_join(fit_real |> 
  spread_draws(cyto_chrom_diff_cell_line_n_min_1[cell_line,protein_id]) |>
  summarise_draws()) |>
...
compare_cell_line_localisation <- protein_dictionary |> 
  inner_join(fit_real |> 
  spread_draws(cyto_chrom_diff_cell_line_n_min_1[protein_id, cell_line]) |>
  summarise_draws()) |> 
  inner_join(cell_line_dictionary) |>
  mutate(Uniprot = ifelse(str_detect(ProteinGroup, ";"),
                               str_extract(ProteinGroup, "[A-Z0-9-]+(?=;)"),
...
ggplot(compare_localisation) +
  geom_boxplot(aes(x = final.assignment, y=log((chrom_intensity_protein)/(cyto_intensity_protein)))) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.x = element_blank(),
        panel.grid.minor.y = element_blank()) +
