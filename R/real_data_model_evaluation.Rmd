...
named_protein_results <- protein_dictionary |> 
  inner_join(fit_real |> 
  spread_draws(cyto_chrom_diff_cell_line[cell_line,protein_id]) |>
  summarise_draws()) |>
...
ggplot(named_protein_results) + 
 ...
 base_protein_results <- protein_dictionary |> 
  inner_join(fit_real |> 
  spread_draws(cyto_intensity_temp[protein_id],
               chrom_intensity_temp[protein_id]) |>
  summarise_draws())

variance_protein_results <- protein_dictionary |> 
  inner_join(fit_real |> 
  spread_draws(sigma_total_intensity[protein_id]) |>
  summarise_draws())
  
ggplot(variance_protein_results) + 
  geom_point(aes(x=ProteinGroup, y=log(median)))
...
compare_localisation <- protein_dictionary |> 
  inner_join(fit_real |> 
  spread_draws(cyto_intensity_protein[protein_id],
               chrom_intensity_protein[protein_id]) |>
  summarise_draws()) |> 
...
compare_cell_line_localisation <- protein_dictionary |> 
  inner_join(fit_real |> 
  spread_draws(cyto_chrom_diff_cell_line[protein_id, cell_line]) |>
  summarise_draws()) |> 
  inner_join(cell_line_dictionary) |>
  mutate(Uniprot = ifelse(str_detect(ProteinGroup, ";"),
                               str_extract(ProteinGroup, "[A-Z0-9-]+(?=;)"),
  ...
  transmute(Uniprot,
            ...
            median,
            variable) |>
  inner_join(compare_localisation)

ggplot(compare_cell_line_localisation |>
         filter(cell_line_name %in% c("cancer_a172", "cancer_a375", "cancer_a549"))) +
  geom_boxplot(aes(x = final.assignment, y=log((cyto_intensity_protein-median)/(chrom_intensity_protein+median)))) +
  theme_bw() +
...
ggplot(compare_localisation) +
  geom_boxplot(aes(x = final.assignment, y=log(cyto_intensity_protein))) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.x = element_blank(),
        panel.grid.minor.y = element_blank())
...
ggplot(compare_localisation) +
  geom_boxplot(aes(x = final.assignment, y=log(chrom_intensity_protein))) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.x = element_blank(),
        panel.grid.minor.y = element_blank())
