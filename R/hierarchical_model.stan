...
model {
  ...
  for(t in 1:tissues){
    mu_cell_line[t,] ~ normal(15,1);
    cyto_extra[t,] ~ normal(1,1);
    
    for(c in 1:cell_lines){ 
        
      //mu_cyto[t,c] ~ normal(mu_cell_line[t,c],1); 
      //mu_chrom[t,c] ~ normal(mu_cell_line[t,c] ,1); 
      chrom_C[t,c,] ~ lognormal(mu_cell_line[t,c] + mu_tissue[t], 1);
      cyto_C[t,c,] ~ lognormal(mu_cell_line[t,c] + mu_tissue[t] + cyto_extra[t,c], 1); 
    }
  }
